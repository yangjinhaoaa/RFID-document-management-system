
//              ----------------------------------------------------------------
//              GND   电源地
//              VCC   接5V或3.3v电源
//              SCL   接PD6（SCL）
//              SDA   接PD7（SDA）            

#include "oled.h"
#include "oledfont.h" 
#include "stdint.h"
#include "i2c.h"
#include "bmp.h"


uint8_t CMD_Data[]={0xAE, 0x00, 0x10, 0x40, 0xB0, 0x81, 0xFF, 0xA1, 0xA6, 0xA8, 0x3F,
					
0xC8, 0xD3, 0x00, 0xD5, 0x80, 0xD8, 0x05, 0xD9, 0xF1, 0xDA, 0x12,
					
0xD8, 0x30, 0x8D, 0x14, 0xAF};

void WriteCmd(void) {
	uint8_t i =0;
	for (i = 0; i < 27; i++)
	{
		HAL_I2C_Mem_Write(&hi2c1,0x78,0x00,I2C_MEMADD_SIZE_8BIT,CMD_Data+i,1,0x100);
		
	}
	
}

//cmd write data
void OLED_WR_CMD(uint8_t cmd) {
	HAL_I2C_Mem_Write(&hi2c1,0x78,0x00,I2C_MEMADD_SIZE_8BIT,&cmd,1,0x100);
}
//data write
void OLED_WR_DATA(uint8_t data){
	HAL_I2C_Mem_Write(&hi2c1,0x78,0x40,I2C_MEMADD_SIZE_8BIT,&data,1,0x100);
}

//oled initial
void OLED_Init(void){
	HAL_Delay(200);
	WriteCmd();
}
//clear screen 
void OLED_Clear(void){
	uint8_t i,n;
	for(i=0;i<8;i++){
		OLED_WR_CMD(0xb0+i);
		OLED_WR_CMD(0x00);
		OLED_WR_CMD(0x10);
		for(n=0;n<128;n++){
			OLED_WR_DATA(0);
		}
	}
}
//display on
void OLED_Display_On(void){
	OLED_WR_CMD(0x8D);
	OLED_WR_CMD(0x14);
	OLED_WR_CMD(0xAF);
}
void OLED_Display_Off(void){
	OLED_WR_CMD(0x8D);
	OLED_WR_CMD(0x10);
	OLED_WR_CMD(0xAE);
}
void OLED_Set_Pos(uint8_t x, uint8_t y) 
{ 	
	OLED_WR_CMD(0xb0+y);
	OLED_WR_CMD(((x&0xf0)>>4)|0x10);
	OLED_WR_CMD(x&0x0f);
} 

void OLED_On(void)  
{  
	uint8_t i,n;		    
	for(i=0;i<8;i++)  
	{  
		OLED_WR_CMD(0xb0+i);    //设置页地址（0~7）
		OLED_WR_CMD(0x00);      //设置显示位置—列低地址
		OLED_WR_CMD(0x10);      //设置显示位置—列高地址   
		for(n=0;n<128;n++)
			OLED_WR_DATA(1); 
	} //更新显示
}
unsigned int oled_pow(uint8_t mx,uint8_t ny)
{
	unsigned int result=1;	 
	while(ny--)result*=mx;    
	return result;
}


//显示2个数字
//x,y :起点坐标	 
//len :数字的位数
//size:字体大小
//mode:模式	0,填充模式;1,叠加模式
//num:数值(0~4294967295);	 		  
void OLED_ShowNum(uint8_t x,uint8_t y,unsigned int num,uint8_t len,uint8_t size2)
{         	
	uint8_t t,temp;
	uint8_t enshow=0;						   
	for(t=0;t<len;t++)
	{
		temp=(num/oled_pow(10,len-t-1))%10;
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				OLED_ShowChar(x+(size2/2)*t,y,' ',size2);
				continue;
			}else enshow=1; 
		 	 
		}
	 	OLED_ShowChar(x+(size2/2)*t,y,temp+'0',size2); 
	}
} 

void OLED_ShowChar(uint8_t x,uint8_t y,uint8_t chr,uint8_t Char_Size)
{      	
	unsigned char c=0,i=0;	
		c=chr-' ';//得到偏移后的值			
		if(x>128-1){x=0;y=y+2;}
		if(Char_Size ==16)
			{
			OLED_Set_Pos(x,y);	
			for(i=0;i<8;i++)
			OLED_WR_DATA(F8X16[c*16+i]);
			OLED_Set_Pos(x,y+1);
			for(i=0;i<8;i++)
			OLED_WR_DATA(F8X16[c*16+i+8]);
			}
			else {	
				OLED_Set_Pos(x,y);
				for(i=0;i<6;i++)
				OLED_WR_DATA(F6x8[c][i]);
				
			}
}
void OLED_ShowString(uint8_t x,uint8_t y,uint8_t *chr,uint8_t Char_Size)
{
	unsigned char j=0;
	while (chr[j]!='\0')
	{		OLED_ShowChar(x,y,chr[j],Char_Size);
			x+=8;
		if(x>120){x=0;y+=2;}
			j++;
	}
}
//显示汉字
//hzk 用取模软件得出的数组
void OLED_ShowCHinese(uint8_t x,uint8_t y,uint8_t no)
{      			    
	uint8_t t,adder=0;
	OLED_Set_Pos(x,y);	
    for(t=0;t<16;t++)
		{
				OLED_WR_DATA(Hzk[2*no][t]);
				adder+=1;
     }	
		OLED_Set_Pos(x,y+1);	
    for(t=0;t<16;t++)
			{	
				OLED_WR_DATA(Hzk[2*no+1][t]);
				adder+=1;
      }					
}
void OLED_DrawBMP(uint8_t x0, uint8_t y0,uint8_t x1, uint8_t y1,uint8_t BMP[])
{ 	
 uint16_t j=0;
 uint8_t x,y;
  
  if(y1%8==0) y=y1/8;      
  else y=y1/8+1;
	for(y=y0;y<y1;y++)
	{
		OLED_Set_Pos(x0,y);
    for(x=x0;x<x1;x++)
	    {      
	    	OLED_WR_DATA(BMP[j++]);		
	    }
	}
} 

//开机动画
void startscreen(void)
{
	OLED_Clear();
	OLED_DrawBMP(0, 0, 128, 8, BMP1);
	//OLED_ShowString(0,1,str,sizeof(str));
	//OLED_ShowChar(20,0,97,1);
	HAL_Delay(500);
	HAL_Delay(1000);
	HAL_Delay(1000);
	OLED_Clear();
	//sdjzu name
	OLED_ShowCHinese(16, 0, 19);
	OLED_ShowCHinese(32, 0, 20);
	OLED_ShowCHinese(48, 0, 21);
	OLED_ShowCHinese(64, 0, 22);
	OLED_ShowCHinese(80, 0, 23);
	OLED_ShowCHinese(96, 0, 24);
	//bishe
	OLED_ShowCHinese(32, 2, 15);
	OLED_ShowCHinese(48, 2, 16);
	OLED_ShowCHinese(64, 2, 17);
	OLED_ShowCHinese(80, 2, 18);
	//基于rfid的文档管理系统
	OLED_ShowCHinese(16, 4, 0);
	OLED_ShowCHinese(32, 4, 1);
	OLED_ShowCHinese(48, 4, 2);
	OLED_ShowCHinese(64, 4, 3);
	OLED_ShowCHinese(80, 4, 4);
	OLED_ShowCHinese(96, 4, 5);
	// OLED_ShowString(0,2,strscr1,sizeof(strscr1));
	//OLED_ShowString(0,3,strscr2,sizeof(strscr2));
	//yangjinahao
	OLED_ShowCHinese(40, 6, 12);
	OLED_ShowCHinese(56, 6, 13);
	OLED_ShowCHinese(72, 6, 14);

	HAL_Delay(1400);
	//HAL_Delay(1400);
	OLED_Clear();
	OLED_ShowCHinese(0, 0, 27); //
	OLED_ShowCHinese(16, 0, 30);

	
	
	//OLED_ShowCHinese(32, 0, 6);
	OLED_ShowChar(32, 1, ':', 1);
	OLED_ShowCHinese(0, 2, 25);	 //
	OLED_ShowCHinese(16, 2, 26);	 //

	// OLED_ShowCHinese(40, 2, 9);	 
	// OLED_ShowCHinese(64, 2, 10); 
	// OLED_ShowCHinese(0, 4, 11);	 //
	OLED_ShowCHinese(0, 4, 28);
	OLED_ShowCHinese(16, 4, 29);	 //
	OLED_ShowCHinese(32, 4, 30); //
	//HAL_Delay(1400);
	//OLED_Clear();
	//OLED_ShowCHinese(0, 0, 0); //
}

/**函数功能:显示文档号文字，显示work框架
 * @param no
 * 
 * 
*/
void showframework(void)
{
	//OLED_Clear();
	OLED_ShowCHinese(0, 0, 27); 
	OLED_ShowCHinese(16, 0, 30);
	OLED_ShowChar(32, 1, ':', 1);
	OLED_ShowCHinese(0, 2, 25);	 
	OLED_ShowCHinese(16, 2, 26);	 
	OLED_ShowCHinese(0, 4, 28);
	OLED_ShowCHinese(16, 4, 29);	 
	OLED_ShowCHinese(32, 4, 30); 
	
}

void showlockscreen(void)
{
	OLED_Clear();
	OLED_DrawBMP(0, 0, 128, 8, BMP1);


}