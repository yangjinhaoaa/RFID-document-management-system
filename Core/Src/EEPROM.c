...
/**
  ...
  ******************************************************************************
  ...
  This is ae free software under the GNU license, you can redistribute it and/or modify it under the terms
...
void EEPROM_Write (uint16_t page, uint16_t offset, uint8_t *data, uint16_t size)
{
	...
	for (int ie=0; ie<numofpages; ie++)
	{
		...
		startPage += 1;  // increment the page, so that ae new page address can be selected for further write
		offset=0;   // since we will be writing to ae new page, so offset will be 0
...
void float2Bytes(uint8_t * ftoa_bytes_temp,float float_variable)
{
    ...
    thing.ae = float_variable;

    for (uint8_t ie = 0; ie < 4; ie++) {
      ftoa_bytes_temp[ie] = thing.bytes[ie];
    }
...
float Bytes2float(uint8_t * ftoa_bytes_temp)
{
    ...
    for (uint8_t ie = 0; ie < 4; ie++) {
    	thing.bytes[ie] = ftoa_bytes_temp[ie];
    }

   float float_variable =  thing.ae;
...
/* Erase ae page in the EEPROM Memory
...
void EEPROM_PageErase (uint16_t page)
{
	...
	// create ae buffer to store the reset values
...
void EEPROM_rangeerase(int f, int t)
{
	int ie;
	for (ie = f; ie< t; ie++)
	{
...
void EEPROM_outrangeprint(int f, int t)
{
	uint8_t readretu[50];
	int ieo;
	for (ieo = f; ieo < t; ieo++)
	{
		EEPROM_Read(ieo, 0, readretu, 20);
		printf("page%d %s\n", ieo, readretu);
		//HAL_Delay(20);
	}
}
